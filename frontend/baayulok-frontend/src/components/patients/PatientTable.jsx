...
export default function PatientTable({ patients, loading, onStatusChange }) {
          ...
          {patients.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid #E2DDD6' }}>
              ...
              <td style={{ padding: '12px 16px' }}>
                ...
                {p.bipannaVerified && (
                  <div style={{ fontSize: '11px', color: '#1D9E75', fontWeight: 600, marginTop: '4px' }}>
                    ✓ Bipanna
                  </div>
                )}
              </td>
              ...
              <td style={{ padding: '12px 16px', whiteSpace: 'nowrap' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <Button variant="secondary" size="sm" onClick={() => navigate(`/patients/${p.id}`)}>
                    View
                  </Button>
                  <Button variant="secondary" size="sm" onClick={() => navigate(`/patients/${p.id}/edit`)}>
                    Edit
                  </Button>
                </div>
              </td>
